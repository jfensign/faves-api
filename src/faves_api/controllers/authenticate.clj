...
(defn- malformed?
 ^{:private true
   :doc "Ensure authorization header is present and set ::username and ::password for subsequent handlers."}
 [_]
...
(defn- authorized?
	^{:private true
		 :doc "Authorize user with [username|email]:password."}
...
(defn- processable?
 ...
 (let [auth-map (-> {} (assoc :remote_addr ip :users_id uid) authenticate/create)
 	...
 	[(not-empty token) {
   ::response      (ring-response {:session {:access_token token} :body {:access_token token}})
